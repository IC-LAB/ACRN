...
def get_optimizer_differ(model, args):
    # 返回optimizer, 不同层设置不同。

    gt_s2_params =  list(map(id, model.module.gt_s2.parameters()))
    gt_s3_params =  list(map(id, model.module.gt_s3.parameters()))
    gt_s4_params =  list(map(id, model.module.gt_s4.parameters()))
    gt_s5_params =  list(map(id, model.module.gt_s5.parameters())) 
    
    timee = args.lr // args.gt_lr
    base_params = filter(lambda p: id(p) not in gt_s2_params + gt_s3_params + gt_s4_params + gt_s5_params, model.parameters())
    params = [{'params': base_params, 'lr': args.lr},
            {'params': model.module.gt_s2.parameters(), 'lr': args.lr/timee},
            {'params': model.module.gt_s3.parameters(), 'lr': args.lr/timee},
            {'params': model.module.gt_s4.parameters(), 'lr': args.lr/timee},
            {'params': model.module.gt_s5.parameters(), 'lr': args.lr/timee},
            ]
    # optimizer = torch.optim.SGD(params, lr=args.lr, momentum=0.9)


    if args.optimizer == 'sgd':
        return torch.optim.SGD(params, args.lr,
                               momentum=args.momentum, nesterov=args.nesterov,
                               weight_decay=args.weight_decay)
    elif args.optimizer == 'rmsprop':
        return torch.optim.RMSprop(params, args.lr,
                                   alpha=args.alpha,
                                   weight_decay=args.weight_decay)
    elif args.optimizer == 'adam':
        return torch.optim.Adam(params, args.lr,
                                beta=(args.beta1, args.beta2),
                                weight_decay=args.weight_decay)
    else:
        raise NotImplementedError
    ...
    if args.gt_lr!=0:
        ...
        for i,param_group in enumerate(optimizer.param_groups):
            if i == 0:
                ...
                param_group['lr'] = lr/timee
            # print('adjust lr ', i, param_group['lr'])
    else:
...
class Trainer(object):
    ...
    def train(self, train_loader, epoch): 
        ...
        lr = adjust_learning_rate(self.optimizer, self.args.lr, self.args.decay_rate, epoch, self.args.epochs, self.args)  # TODO: add custom
    ...
    def test(self, val_loader, epoch, silence=False): 
        ...
        # evaluator_seg_class = Evaluator(num_class=2)
        # evaluator_seg_class.reset()
        evaluator_seg_class = AverageMeter() 
