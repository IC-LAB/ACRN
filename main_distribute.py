...
from model import *  
from trainer import Trainer,get_optimizer,get_optimizer_differ
...
args.train_from_seg_pt = False # 加载保存的分割模型

# pt_path_seg = "./model/camelyon512_Botnet_cls_stainnorm_w0_5_lr0.1_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/camelyon512_FPN_seg_stainnorm_w0_5_lr0.1_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/camelyon512_Botnet_seg_stainnorm_w0_5_lr0.01_T60_mean0.5_resize_pretrain/model_best.pth.tar" 

# pt_path_seg = "./model/paip512_Botnet_cls_stainnorm_w0_5_lr0.01_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/paip512_FPN_seg_stainnorm_w0_5_lr0.1_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/paip512_Botnet_seg_stainnorm_w0_5_lr0.01_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/paip512_FPT_seg_stainnorm_w0_5_lr0.1_T60_mean0.5_resize_pretrain/model_best.pth.tar" 
# pt_path_seg = "./model/paip512_FPT_Botnet_seg_stainnorm_w0_5_lr0.1_T60_mean0.5_resize_pretrainfrombotFPT/model_best.pth.tar" 


test_code = False
args.distribute = True

args.model_name = 'cls_T_seg_S' # vgg 改lr=0.01
args.lubda = 1

args.transform_input = False  # false==>0.5
args.stainnorm = True

data_selcect = 'paip512' # paip512 , camelyon512 

args.gpu_number = '4,5,6,7' 

args.resize = True
bs = 64//4 # 没有resize时的需要设置batchsize # distribute的一张卡， parrall的全部

args.lr = 0.1 # cls=0.1 , fpn=0.01  
args.gt_lr = 0.0001 # 没用置零
...
args.cls = True if '_cls' in args.model_name or 'cls_' in args.model_name else False
args.res = True if '_res' in args.model_name else False
args.seg = True if '_seg' in args.model_name else False 
#==== model path- pretrain===============
if args.resize: 
    args.pt_path = "./model/resnet50_cls_camelyon256.pth.tar"  if data_selcect=='camelyon512' else "./model/resnet50_cls_paip256.pth.tar"
    if 'Bot' in args.model_name and args.pretrained: 
        args.pt_path = "./model/Botnet50_cls_camelyon256.pth.tar" if data_selcect=='camelyon512' else  "./model/Botnet50_cls_paip256.pth.tar" # 加载模型路径
    if args.distribute:
        args.batch_size = 64//args.gpus if 'FPT' in args.model_name else 128//args.gpus # total 128
    else:
        args.batch_size= 128  
elif not args.resize:
    args.pt_path = "./model/resnet50_cls_camelyon512.pth.tar" if data_selcect=='camelyon512' else "./model/resnet50_cls_paip512.pth.tar"
    args.batch_size= bs
if args.train_from_seg_pt:
    args.pt_path = pt_path_seg   

#====data path root======
if data_selcect == 'camelyon512':
    args.img_root = '../../../camelyon16/figure_512_tile_random_stainnorm'  if args.stainnorm else '../../../camelyon16/figure_512_tile_random' 
elif data_selcect == 'paip512':
    args.img_root = '../../../PAIP2019Challenge/figure_512_tile_stainnorm' if args.stainnorm else '../../../PAIP2019Challenge/figure_512_tile' 
logroot = './logs/camelyon512/with_0_5/'  if data_selcect=='camelyon512' else  './logs/paip512/with_0_5/'  

#=====log path==========
logname = data_selcect + '_' + args.model_name + '_stainnorm_w0_5_lr'+str(args.lr)+'_T'+str(args.epochs) if args.stainnorm else data_selcect + '_' + args.model_name + '_w0_5_lr'+str(args.lr)+'_T'+str(args.epochs) 
logname = logname + '_mean' if args.transform_input else logname + '_mean0.5'
logname = logname + '_resize' if args.resize else logname
logname = logname + '_pretrain' if args.pretrained else logname
logname = logname + 'fromseg' if args.train_from_seg_pt else logname
# logname = logname + '_lubda'+str(args.lubda)
if args.gt_lr!=0:
    logname = logname + '_gt'+str(args.gt_lr)
...
args.train_data_dir = data_root + 'train/train_with_0_5.csv' if not test_code else './test_code.csv'
args.val_data_dir = data_root +'val/val_with_0_5_sample.csv' if not test_code else './test_code.csv'
args.test_data_dir = data_root + 'val/val_with_0_5.csv'  if not test_code else './test_code.csv'
...
def main():
    ...
    model = get_model(model_name=args.model_name,transform_input=args.transform_input, resolution=(args.resize_size,args.resize_size), img_root = args.img_root)
    if args.distribute:
        model.to(device)
    ...
    # define optimizer  
    if args.gt_lr!=0:
        optimizer = get_optimizer_differ(model, args)    
    else:
            ...
            if args.local_rank==0:
                ...
                if args.cls and not args.res and args.seg: # cls + seg
                    log_print('test:lr{lr}, loss{loss:.4f}, loss_cls{loss_cls:.4f}, loss_seg{loss_seg:.4f}, \
                            cls_acc{cls_acc:.4f},seg_cls_acc{seg_cls_acc:.4f},seg_acc{seg_acc:.4f},mIoU{mIoU:.4f}'\
                            .format(lr=lr, loss=test_loss, loss_cls=test_loss_cls, loss_seg=test_loss_seg,
                            cls_acc=test_cls_acc, seg_cls_acc=test_seg_cls_acc, seg_acc=test_seg_acc, mIoU=test_seg_mIoU))
                elif args.cls and args.res and not args.seg: # cls + res:
                    ...
                    print(Fore.RED + 'test:lr{lr}, loss{loss:.4f}, seg_cls_acc{class_acc:.4f},seg_acc{seg_acc:.4f},mIoU{mIoU:.4f}'\
                            .format(lr=lr,loss=test_loss, class_acc=test_seg_cls_acc,seg_acc=test_seg_acc, mIoU=test_seg_mIoU))
                else:
                    log_print('cls res seg error') 
                print(Fore.RESET)
    ...
    if args.local_rank==0:
        ...
        os.remove(os.path.join(args.log_dir, 'model_last.pth.tar')) 
